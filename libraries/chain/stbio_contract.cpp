...
void validate_authority_precondition( const apply_context& context, const authority& auth ) {
   for(const auto& a : auth.accounts) {
      ...
      if( a.permission.permission == config::eosio_code_name ) // virtual stbio.code permission does not really exist but is allowed
         continue;
 ...
 */
void apply_stbio_newaccount(apply_context& context) {
...
//   context.require_write_lock( config::eosio_auth_scope );
   ...
   if( !creator.is_privileged() ) {
      EOS_ASSERT( name_str.find( "stb." ) != 0, action_validate_exception,
                  "only privileged accounts can have names that start with 'stb.'" );
   }
...
void apply_stbio_setcode(apply_context& context) {
...
void apply_stbio_setabi(apply_context& context) {
   auto& db  = context.db;
...
void apply_stbio_updateauth(apply_context& context) {
   ...
   EOS_ASSERT( update.permission.to_string().find( "stb." ) != 0, action_validate_exception,
               "Permission names that start with 'stb.' are reserved" );
...
void apply_stbio_deleteauth(apply_context& context) {
//   context.require_write_lock( config::eosio_auth_scope );
...
void apply_stbio_linkauth(apply_context& context) {
//   context.require_write_lock( config::eosio_auth_scope );
...
void apply_stbio_unlinkauth(apply_context& context) {
//   context.require_write_lock( config::eosio_auth_scope );
...
void apply_stbio_canceldelay(apply_context& context) {
