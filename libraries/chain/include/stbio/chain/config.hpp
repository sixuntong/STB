...
const static name system_account_name    { N(stbio) };
const static name null_account_name      { N(stb.null) };
const static name producers_account_name { N(stb.prods) };

// Active permission of producers account requires greater than 2/3 of the producers to authorize
const static name majority_producers_permission_name { N(stb.major) }; // greater than 1/2 of producers needed to authorize
const static name minority_producers_permission_name { N(stb.minor) }; // greater than 1/3 of producers needed to authorize0

const static name eosio_auth_scope       { N(stb.auth) };
const static name eosio_all_scope        { N(stb.all) };

const static name active_name     { N(active) };
const static name owner_name      { N(owner) };
const static name eosio_any_name  { N(stb.any) };
const static name eosio_code_name { N(stb.code) };
