...
authority parse_json_authority_or_key(const std::string& authorityJsonOrFile) {
   if (boost::istarts_with(authorityJsonOrFile, "STB") || boost::istarts_with(authorityJsonOrFile, "PUB_R1")) {
      try {
   ...
   bidname_info_subcommand(CLI::App* actionRoot) {
      ...
      list_producers->callback([this] {
         auto rawResult = call(get_table_func, fc::mutable_variant_object("json", true)
                               ("code", "stbio")("scope", "stbio")("table", "namebids")
...
//                                ->check(CLI::ExistingFile);
   ...
   propose_action->callback([&] {
      ...
      send_actions({chain::action{accountPermissions, N(stb.msig), N(propose), variant_to_bin( N(stb.msig), N(propose), args ) }});
   });
   ...
   auto approve_or_unapprove = [&](const string& action) {
      ...
      send_actions({chain::action{accountPermissions, N(stb.msig), name(action), variant_to_bin( N(stb.msig), name(action), args ) }});
   };
   ...
   invalidate->callback([&] {
      ...
      send_actions({chain::action{accountPermissions, N(stb.msig), N(invalidate), variant_to_bin( N(stb.msig), N(invalidate), args ) }});
   });
   ...
   cancel->callback([&]() {
      ...
      send_actions({chain::action{accountPermissions, N(stb.msig), N(cancel), variant_to_bin( N(stb.msig), N(cancel), args ) }});
      }
   );
   ...
   exec->callback([&] {
      ...
      send_actions({chain::action{accountPermissions, N(stb.msig), N(exec), variant_to_bin( N(stb.msig), N(exec), args ) }});
      }
   );
   ...
   // system subcommand
   auto system = app.add_subcommand("system", localized("Send stb.system contract action to the blockchain."));
